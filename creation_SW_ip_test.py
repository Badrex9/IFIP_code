...
# Hachage basé sur IP source, IP destination et port destination
def hash_ip_port(ip_src, ip_dst, port_dst):
    """
    Crée un hash unique à partir de l'IP source, de l'IP destination, et du port destination.
    Le couple IP est trié pour ignorer le sens du flux.
    """
    min_ip = min(ip_src, ip_dst)
    max_ip = max(ip_src, ip_dst)
    hash_input = f"{min_ip}{max_ip}{port_dst}"
...
d_historique = 20
X_input = transfo(X_test, source_ip_test.astype(int), dest_ip_test.astype(int), dest_port_test.astype(int))


split_npy_save(X_input, 20, 'X_input_split_test')
